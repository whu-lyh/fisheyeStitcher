...
namespace stitcher
{
	class FISHEYE_STITCHER_API FisheyeStitcher
	{
	public:
		FisheyeStitcher(int width, int height, float in_fovd, bool enb_light_compen, bool enb_refine_align, std::string map_path);
		~FisheyeStitcher();
	...
	private:
		...
		cv::Point2f findMatchLoc(const cv::Mat &Ref, const cv::Mat &Tmpl,
			const std::string &img_window, const bool disable_display);

		std::tuple<std::vector<cv::Point2f>, std::vector<cv::Point2f> >
			createControlPoints(const cv::Point2f &matchLocLeft, const cv::Point2f &matchLocRight, const int row_start,
