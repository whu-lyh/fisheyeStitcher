...
namespace stitcher
{
	...
	//!
	cv::Mat FisheyeStitcher::unwarp(const cv::Mat &src)
	{
	...
	//! 
	std::tuple<double, double> FisheyeStitcher::fish2Eqt(const double x_dest, const double y_dest, const double W_rad)
	{
	...
	//! 
	void FisheyeStitcher::createMask()
	{
	...
	//!
	cv::Mat FisheyeStitcher::deform(const cv::Mat &src)
	{
		cv::Mat dst(src.size(), src.type());
	...
	//! 
	void FisheyeStitcher::genScaleMap()
	{
	...
	//!
	cv::Mat FisheyeStitcher::compenLightFO(const cv::Mat &in_img)
	{
		cv::Mat rgb_ch[3];
	...
	//!     Update member masks.
	//!
	void FisheyeStitcher::createBlendMask()
	{
	...
	//!
	cv::Point2f FisheyeStitcher::findMatchLoc(const cv::Mat &Ref,
			const cv::Mat &Tmpl,
	...
	//! 
	cv::Mat FisheyeStitcher::blendRight(const cv::Mat &bg1, const cv::Mat &bg2)
	{
	...
	//! @param  return  blended patch
	//! 
	cv::Mat FisheyeStitcher::blendLeft(const cv::Mat &bg1, const cv::Mat &bg2)
	{
		int h = bg1.size().height;
		int w = bg1.size().width;
		double wdb = static_cast<double>(w);
	...
	//! 
	cv::Mat FisheyeStitcher::blend(const cv::Mat &left_img,
	...
	//!
	cv::Mat FisheyeStitcher::stitch(const cv::Mat& in_img_L, const cv::Mat& in_img_R)
	{
		// int W_in = 1920;
